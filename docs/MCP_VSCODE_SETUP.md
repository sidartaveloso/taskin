...
| `leave_group` | Takes a task out of its group — only offered when the provider has groups |
| `create_group` | Creates a group, at the root or inside another one (`parentId`); `id` is optional. Groups nest at most four levels deep — only offered when the provider has groups |
| `nest_group`  | Puts a group inside another (`groupId`, `parentId`), its subgroups along; its tasks are not touched. A missing parent, a cycle and more than four levels are refused — only offered when the provider has groups inside groups |
| `unnest_group` | Takes a group out of its parent, back to the root — offered with `nest_group` |
| `move_group`  | Moves a whole group: exactly one of `before` / `after` (a task or another group), `top: true` or `bottom: true`. Only the members are written — only offered when the provider has groups |
