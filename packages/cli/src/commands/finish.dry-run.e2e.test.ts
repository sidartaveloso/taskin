...
beforeEach(() => {
  raiz = mkdtempSync(join(tmpdir(), 'taskin-finish-dry-run-'));
...
/**
 * O `--dry-run` de finish precisa provar duas coisas: que anuncia o que faria,
 * e que nao faz nada. Ver `finish.no-skip-ci.e2e.test.ts` como forma.
 */
describe('taskin finish --dry-run', () => {
  it('anuncia a transicao para done e o commit que faria', async () => {
    const saida = await rodar('finish', '001', '--dry-run');

    expect(saida).toContain('Dry run');
    expect(saida).toContain('→ done');
    expect(saida).toContain('task-001');
  });

  it('nao altera o status da task no arquivo', async () => {
    await rodar('finish', '001', '--dry-run');
