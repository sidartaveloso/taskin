<template>
  <Teleport to="body">
    <div class="gesture-wizard-overlay">
        ...
        <div class="wizard-body" v-else-if="wizardState === 'READY'">
          ...
          <div class="progress-track">
            <div
              ...
              :style="{
                width:
                  Math.min(100, ((readyProgress - 2000) / 3000) * 100) + '%',
              }"
            />
          </div>
          <p class="hint">
        ...
        <div class="wizard-body" v-else-if="wizardState === 'RECORDING'">
          ...
          <div class="gesture-preview" v-if="recordingCandidate">
            <span class="preview-emoji">{{
              gestureEmoji[recordingCandidate]
            }}</span>
            <span class="preview-label">{{
              gestureLabel[recordingCandidate]
            }}</span>
          </div>
        </div>
        ...
        <div class="wizard-body" v-else-if="wizardState === 'CONFIRMING'">
          ...
          <div class="confirm-pair">
            <div class="confirm-item">
              <span class="confirm-emoji">{{
                gestureEmoji[lastMapping?.gesture || 'None']
              }}</span>
            ...
            <div class="confirm-item">
              <span class="confirm-label">{{
                actionLabel[lastMapping?.action || 'none']
              }}</span>
            </div>
          </div>
          <div class="wizard-legend">
...
@keyframes pulseGlow {
  ...
  50% {
    filter: drop-shadow(0 0 16px #4fc3f7);
  }
}
