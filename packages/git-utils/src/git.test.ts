...
import { branchExists, checkoutBranch, createBranch, getCurrentBranch, isGitRepository } from './git';
...
describe('Git Utilities', () => {
  ...
  it('should confirm a branch exists', () => {
    ...
    expect(execSync).toHaveBeenCalledWith('git show-ref --verify refs/heads/main', expect.any(Object));
  });
  ...
  it('should create a new branch', () => {
    ...
    expect(execSync).toHaveBeenCalledWith('git checkout -b new-feature main', expect.any(Object));
  });

  it('should checkout an existing branch', () => {
    checkoutBranch('existing-feature');
    expect(execSync).toHaveBeenCalledWith('git checkout existing-feature', expect.any(Object));
  });
});
