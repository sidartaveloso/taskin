...
- [x] Verificar no dashboard aberto do `.bench500` nas duas telas — feito fora do sandbox. `?view=board&filter=all&q=carga%2012&sort=diff-desc&score=all`: o Board mostrou 11 cards, o titulo `All tasks` e a ordem vinda da URL; trocar para Prioritization e recarregar manteve os cinco parametros, a busca preenchida e as mesmas 11 tasks. Titulos conferidos: `Open tasks`, `All tasks`. As stories da `PrioritizationPage` passaram no Chromium (5/5). A verificacao achou a barra quebrando em duas linhas ja em 1440px e alargando a pagina no celular (501px em 375); corrigido no commit seguinte: uma linha (41px) em 1280 e 1440, duas (70px) em 1024, e 375 sem rolagem horizontal
...
### Achado na verificacao, fora do escopo

Com um recorte que nao tem nenhuma task (por exemplo `Active` no `.bench500`), o
quadro diz "Nenhuma tarefa encontrada — Conecte-se ao servidor para visualizar
suas tarefas", com a conexao verde. A mensagem culpa a conexao quando quem
esvaziou a lista foi o filtro. Registrado na task-130.
